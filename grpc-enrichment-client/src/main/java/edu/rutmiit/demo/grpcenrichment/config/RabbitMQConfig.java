 ...
 *
 * Этот сервис одновременно и consumer (слушает route.created),
 * и publisher (публикует route.enriched). Поэтому конфигурация включает:
 * - Exchange (общий для всех сервисов)
 * - Очередь для приёма route.created
 * - DLQ для необработанных сообщений
 * - RabbitTemplate для публикации route.enriched
 *
 * Каждый consumer определяет свою очередь и привязку.
 * Enrichment-клиент слушает только route.created (не все события),
 * поэтому binding key = "route.created", а не "#".
 */
@Configuration
public class RabbitMQConfig {

    public static final String ENRICHMENT_QUEUE = "q.enrichment.route-created";
    public static final String ENRICHMENT_DLQ = "q.enrichment.route-created.dlq";
    ...
    // Очередь для route.created
    // Только события создания книг — binding key "route.created" (не "#")
    ...
    /**
     * Привязка, только route.created и очередь enrichment.
    ...
    public Binding enrichmentBinding(Queue enrichmentQueue, TopicExchange eventsExchange) {
        return BindingBuilder
                ...
                .to(eventsExchange)
                .with(RoutingKeys.ROUTE_CREATED); // только route.created
    }
