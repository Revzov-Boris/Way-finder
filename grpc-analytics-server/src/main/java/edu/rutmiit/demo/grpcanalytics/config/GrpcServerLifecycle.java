...
import edu.rutmiit.demo.grpcanalytics.service.RouteAnalyticsServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new RouteAnalyticsServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: RouteAnalytics.AnalyzeRoute()");
